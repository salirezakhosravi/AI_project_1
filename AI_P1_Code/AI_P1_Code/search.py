...
def depthFirstSearch(problem: SearchProblem) -> List[Directions]:
    ...
    while not fringe.isEmpty():
       ...
       if problem.isGoalState(current_state):
           return actions
       elif current_state not in visited_nodes:
...
def uniformCostSearch(problem: SearchProblem) -> List[Directions]:
    ...
    visited_nodes = []
    actions = []
    fringe.push((problem.getStartState(), actions), problem.getCostOfActions(actions))
       ...
       if problem.isGoalState(current_state):
           return actions
            ...
            for next_state, action, stepCost in problem.getSuccessors(current_state):
                new_actions = list(actions)
                new_actions.append(action)
                fringe.push((next_state, new_actions), problem.getCostOfActions(new_actions))
...
def aStarSearch(problem: SearchProblem, heuristic=nullHeuristic) -> List[Directions]:
    ...
    fringe.push((start_state, actions, 0), heuristic(start_state, problem))
    visited_nodes[start_state] = 0
    ...
    while not fringe.isEmpty():
        ...
        for next_state, action, stepCost in problem.getSuccessors(current_state):
            ...
            new_actions = list(actions)
            new_actions.append(action)
            if next_state not in visited_nodes or new_cost < visited_nodes[next_state]:
                visited_nodes[next_state] = new_cost
